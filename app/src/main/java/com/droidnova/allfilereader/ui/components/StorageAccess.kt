...
fun rememberStorageAccessRequest(onReturn: () -> Unit): () -> Unit {
    ...
    val settingsLauncher = rememberLauncherForActivityResult(ActivityResultContracts.StartActivityForResult()) { onReturn() }
    val permissionLauncher = rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission()) { onReturn() }
    return remember(context, onReturn) {{
