...
@HiltViewModel class PowerPointReaderViewModel @Inject constructor(saved:SavedStateHandle,private val repository:DocumentRepository,private val permissions:MediaPermissionManager,@ApplicationContext context:Context):ViewModel(){
 ...
 private fun legacyPhase(code:LegacyPptErrorCode)=when(code){LegacyPptErrorCode.INPUT_EMPTY,LegacyPptErrorCode.INVALID_OLE_SIGNATURE->PptxPhase.LegacyInvalid;LegacyPptErrorCode.TEMP_COPY_FAILED,LegacyPptErrorCode.INSUFFICIENT_STORAGE,LegacyPptErrorCode.FILE_PROVIDER_FAILED->PptxPhase.LegacyPreparationFailed;else->PptxPhase.LegacyAccessDenied} private suspend fun showLegacyError(code:LegacyPptErrorCode)=withContext(Dispatchers.Main.immediate){_state.update{it.copy(phase=legacyPhase(code))}}
