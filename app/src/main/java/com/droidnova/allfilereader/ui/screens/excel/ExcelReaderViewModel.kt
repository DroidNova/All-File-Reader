...
enum class ExcelReaderPhase { Resolving, Validating, PreparingViewer, ParsingWorkbook, Ready, EmptyWorkbook, MissingFile, PermissionDenied, UnsupportedFormat, PasswordProtected, FileTooLarge, SafetyLimit, Corrupted, WebViewUnavailable, RendererProcessCrashed, ParseTimeout, ParseFailure }
data class ExcelReady(val file:File,val token:String,val mediaType:String)
...
@HiltViewModel class ExcelReaderViewModel @Inject constructor(saved:SavedStateHandle,private val repository:DocumentRepository,private val permissions:MediaPermissionManager,@ApplicationContext context:Context):ViewModel(){
 ...
 private fun load(){job?.cancel();cleanup();_state.value=ExcelReaderUiState();job=viewModelScope.launch(Dispatchers.IO){try{if(!permissions.isGranted())throw SecurityException();val d=repository.resolveDocument(id)?:throw FileNotFoundException();Log.i(TAG,"resolution=SUCCESS");show(d.displayName,ExcelReaderPhase.Validating);val session=preflight.copyAndValidate(Uri.parse(d.uri),d.mimeType,d.sizeBytes);temp=session.file;val token=ByteArray(24).also(SecureRandom()::nextBytes).joinToString(""){"%02x".format(it)};withContext(Dispatchers.Main.immediate){_state.value=ExcelReaderUiState(d.displayName,ExcelReaderPhase.PreparingViewer,ExcelReady(session.file,token,session.mediaType))}}catch(c:CancellationException){cleanup();throw c}catch(e:Exception){cleanup();val phase=when(e){is SecurityException->ExcelReaderPhase.PermissionDenied;is FileNotFoundException->ExcelReaderPhase.MissingFile;is SpreadsheetPreflightException.TooLarge->ExcelReaderPhase.FileTooLarge;is SpreadsheetPreflightException.Encrypted->ExcelReaderPhase.PasswordProtected;is SpreadsheetPreflightException.Unsupported->ExcelReaderPhase.UnsupportedFormat;is SpreadsheetPreflightException.Corrupted->ExcelReaderPhase.Corrupted;is SpreadsheetPreflightException.Safety->ExcelReaderPhase.SafetyLimit;else->ExcelReaderPhase.ParseFailure};withContext(Dispatchers.Main.immediate){_state.update{it.copy(phase=phase)}}}}}
