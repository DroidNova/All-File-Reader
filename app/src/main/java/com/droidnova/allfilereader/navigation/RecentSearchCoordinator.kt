...
data class SearchActivationRequest(val id: Long, val source: SearchActivationSource)
...
@Singleton class RecentSearchCoordinator @Inject constructor() {
    ...
    fun request(source: SearchActivationSource): Long = ids.incrementAndGet().also { id ->
        _pending.value = SearchActivationRequest(id, source)
    }
