...
class DocumentVisibilityPolicyTest {
 ...
 @Test fun mediaPackagesAndArchivesAreExcluded(){listOf("jpg","png","mp4","mp3","apk","zip","rar","7z","exe","tmp","rtf","epub").forEach{assertFalse(it,DocumentClassifier.isVisibleDocument(DocumentClassifier.classify(null,it)))}}
 @Test fun recognizedUnsupportedOfficeDocumentsStayVisible(){listOf("doc","ppt","pptm","pps","ppsx").forEach{assertTrue(it,DocumentClassifier.isVisibleDocument(DocumentClassifier.classify(null,it)))}}
}
