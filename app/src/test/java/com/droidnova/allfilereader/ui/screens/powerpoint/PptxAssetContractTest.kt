...
class PptxAssetContractTest {
 ...
 @Test fun viewerSecurityAndLifecycleContract(){val html=File(root,"app/src/main/assets/pptx_viewer/viewer.html").readText();val js=File(root,"app/src/main/assets/pptx_viewer/viewer.js").readText();assertTrue(html.contains("Content-Security-Policy"));assertTrue(js.contains("fitMode: "contain""));assertTrue(js.contains("searchText"));assertTrue(js.contains("IntersectionObserver"));assertTrue(js.contains("sliderendered"));assertTrue(js.contains("slideunmounted"));assertTrue(js.contains("return true;"));assertFalse(js.contains("search:async"));assertTrue(js.contains("viewer?.destroy()"));assertFalse(js.contains("http://"));assertFalse(js.contains("https://"))}
}
